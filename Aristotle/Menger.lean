...
/-
We have formally proved Menger's theorem for finite simple graphs. The proof follows the inductive argument from Diestel's Graph Theory. We first established the weak direction (max paths ≤ min separator) as a lemma (assumed given). For the strong direction (min separator ≤ max paths), we used strong induction on the number of edges. The inductive step considers an edge `e = xy`. If contracting `e` preserves the minimum separator size `k`, we lift the `k` disjoint paths from the contracted graph. If contracting `e` reduces the minimum separator size, we find a separator `X` of size `k` containing `x` and `y`, and then use the induction hypothesis on `G-e` to find `k` disjoint paths from `A` to `X` and from `X` to `B`, which are then combined. Helper lemmas were proved to handle the properties of separators and paths under contraction and deletion.
-/
...
-/
instance SimpleGraph.ABPath.instFinite [Fintype V] (G : SimpleGraph V) (A B : Set V) : Finite (G.ABPath A B) := by
...
-/
def SimpleGraph.separators [Fintype V] (G : SimpleGraph V) (A B : Set V) : Finset (Finset V) :=
  (Finset.powerset Finset.univ).filter (fun S => G.Separates A B S)

/-
The set of all sets of disjoint A-B paths.
-/
noncomputable def SimpleGraph.disjoint_path_sets [Fintype V] (G : SimpleGraph V) (A B : Set V) : Finset (Finset (G.ABPath A B)) :=
...
-/
theorem SimpleGraph.Menger_weak [Fintype V] (G : SimpleGraph V) (A B : Set V) :
...
def SimpleGraph.contractEdge_liftSet (x y : V) (S : Set V) : Set (Quotient (SimpleGraph.contractEdgeSetoid x y)) :=
...
-/
def SimpleGraph.contractEdge_preimage [Fintype V] (x y : V) (Y : Finset (Quotient (SimpleGraph.contractEdgeSetoid x y))) : Finset V :=
...
-/
lemma SimpleGraph.contractEdge_preimage_separates [Fintype V] (G : SimpleGraph V) (A B : Set V) (x y : V)
...
-/
lemma SimpleGraph.lift_walk_avoiding_contraction [Fintype V] (G : SimpleGraph V) (x y : V)
...
-/
lemma SimpleGraph.lift_path_avoiding_contraction_AB [Fintype V] (G : SimpleGraph V) (A B : Set V) (x y : V)
  {u v : Quotient (SimpleGraph.contractEdgeSetoid x y)} (p : (G.contractEdge' x y).Walk u v)
  (hp_path : p.IsPath)
...
-/
lemma SimpleGraph.separator_in_G_of_separator_in_G_delete_edge (G : SimpleGraph V) (A B : Set V) (x y : V) (X : Finset V) (S : Finset V)
...
-/
lemma SimpleGraph.disjoint_paths_prop [Fintype V] (G : SimpleGraph V) (A X : Set V) (P : Finset (G.ABPath A X))
...
-/
lemma SimpleGraph.ABPath_prefix_avoids_X (G : SimpleGraph V) (A X : Set V) (X_fin : Finset V)
  (p : G.ABPath A X)
...
-/
lemma SimpleGraph.ABPath_suffix_avoids_X (G : SimpleGraph V) (B X : Set V) (X_fin : Finset V)
  (q : G.ABPath X B)
...
-/
lemma SimpleGraph.path_intersection_of_separator (G : SimpleGraph V) (A B : Set V) (X : Finset V)
...
-/
lemma SimpleGraph.disjoint_paths_prop_start [Fintype V] (G : SimpleGraph V) (X B : Set V) (P : Finset (G.ABPath X B))
  (hP_disj : DisjointPaths P)
  (hP_card : P.card = X.toFinset.card) :
...
-/
lemma SimpleGraph.joined_path_is_path (G : SimpleGraph V) (A B : Set V) (X : Finset V)
  (hX_sep : G.Separates A B X)
  (x : V)
...
-/
lemma SimpleGraph.joined_paths_disjoint (G : SimpleGraph V) (A B : Set V) (X : Finset V)
  (hX_sep : G.Separates A B X)
  (x y : V) (hxy : x ≠ y)
...
-/
theorem SimpleGraph.disjoint_paths_join [Fintype V] (G : SimpleGraph V) (A B : Set V) (X : Finset V)
  (hX_sep : G.Separates A B X)
  (k : ℕ)
  (hX_card : X.card = k)
...
-/
lemma SimpleGraph.lift_path_to_contraction_nil [Fintype V] (G : SimpleGraph V) (A : Set V) (x y : V)
...
-/
lemma SimpleGraph.at_most_one_path_through_vertex (G : SimpleGraph V) (A B : Set V) (P : Finset (G.ABPath A B)) (v : V)
  (hP_disj : DisjointPaths P) :
...
-/
lemma SimpleGraph.exists_lifted_ABPath_avoiding [Fintype V] (G : SimpleGraph V) (A B : Set V) (x y : V)
...
-/
lemma SimpleGraph.exists_disjoint_paths_lift [Fintype V] (G : SimpleGraph V) (A B : Set V) (x y : V) (hxy : G.Adj x y)
...
-/
lemma SimpleGraph.separator_in_G_of_separator_in_G_delete_edge_right (G : SimpleGraph V) (A B : Set V) (x y : V) (X : Finset V) (S : Finset V)
  (hX : G.Separates A B X) (hx : x ∈ X) (hy : y ∈ X) (hxy : x ≠ y)
  ...
  G.Separates A B S := by
    ...
    contrapose! this;
    refine' ⟨ x, y, X, S, _, hx, hy, hxy, _, _ ⟩;
    ...
    · convert this using 1;
      simp +decide [ SimpleGraph.Separates, SimpleGraph.Walk ];
...
-/
lemma SimpleGraph.min_sep_delete_ge_k_left [Fintype V] (G : SimpleGraph V) (A B : Set V) (x y : V) (X : Finset V)
  (k : ℕ)
  (h_min : G.min_separator_size A B = k)
...
-/
lemma SimpleGraph.min_sep_delete_ge_k_right [Fintype V] (G : SimpleGraph V) (A B : Set V) (x y : V) (X : Finset V)
  (k : ℕ)
  (h_min : G.min_separator_size A B = k)
  (hX_sep : G.Separates A B X) (hx : x ∈ X) (hy : y ∈ X) (hxy : x ≠ y) :
...
-/
lemma SimpleGraph.lift_disjoint_paths_le [Fintype V] (G G' : SimpleGraph V) (h : G' ≤ G) (A B : Set V)
  (P : Finset (G'.ABPath A B)) (hP_disj : DisjointPaths P) :
  ∃ Q : Finset (G.ABPath A B), DisjointPaths Q ∧ Q.card = P.card := by
...
-/
lemma SimpleGraph.Menger_case2_imp_paths [Fintype V] (G : SimpleGraph V) [DecidableRel G.Adj] (A B : Set V) (x y : V) (hxy : G.Adj x y)
  (k : ℕ)
  (h_min : G.min_separator_size A B = k)
  (X : Finset V)
  (hX_sep : G.Separates A B X)
  ...
  k ≤ G.max_disjoint_paths_size A B := by
    ...
    obtain ⟨P_A', hP_A'_disj, hP_A'_card⟩ : ∃ P_A' : Finset ((G.deleteEdge x y).ABPath A X), DisjointPaths P_A' ∧ P_A'.card ≥ k := by
      obtain ⟨P_A', hP_A'_disj, hP_A'_card⟩ : ∃ P_A' : Finset ((G.deleteEdge x y).ABPath A X), DisjointPaths P_A' ∧ P_A'.card = (G.deleteEdge x y).max_disjoint_paths_size A X := by
        ...
        exact ⟨ _, Finset.mem_image_of_mem _ ( Finset.mem_filter.mpr ⟨ Finset.mem_univ ∅, by simp +decide [ DisjointPaths ] ⟩ ) ⟩;
      exact ⟨ P_A', hP_A'_disj, hP_A'_card.symm ▸ h_ind.1 ⟩
    obtain ⟨P_B', hP_B'_disj, hP_B'_card⟩ : ∃ P_B' : Finset ((G.deleteEdge x y).ABPath X B), DisjointPaths P_B' ∧ P_B'.card ≥ k := by
      contrapose! h_ind;
    ...
    obtain ⟨P_A, hP_A_disj, hP_A_card⟩ : ∃ P_A : Finset (G.ABPath A X), DisjointPaths P_A ∧ P_A.card = k := by
      have := Finset.exists_subset_card_eq hP_A'_card;
      obtain ⟨ t, ht₁, ht₂ ⟩ := this;
      have h_lift_A : ∃ P_A : Finset (G.ABPath A X), DisjointPaths P_A ∧ P_A.card = t.card := by
        apply_rules [ SimpleGraph.lift_disjoint_paths_le ];
        · intro u v; by_cases hu : u = x <;> by_cases hv : v = y <;> simp +decide [ *, SimpleGraph.deleteEdge ] ;
          · exact?;
          · exact?;
          · exact?;
        · exact fun p hp q hq hpq => hP_A'_disj p ( ht₁ hp ) q ( ht₁ hq ) hpq;
      aesop
    obtain ⟨P_B, hP_B_disj, hP_B_card⟩ : ∃ P_B : Finset (G.ABPath X B), DisjointPaths P_B ∧ P_B.card = k := by
      obtain ⟨P_B'', hP_B''_disj, hP_B''_card⟩ : ∃ P_B'' : Finset ((G.deleteEdge x y).ABPath X B), DisjointPaths P_B'' ∧ P_B''.card = k := by
      ...
      have h_lift : ∃ P_B : Finset (G.ABPath X B), DisjointPaths P_B ∧ P_B.card = k := by
    ...
    obtain ⟨P, hP_disj, hP_card⟩ : ∃ P : Finset (G.ABPath A B), DisjointPaths P ∧ P.card = k := by
...
-/
lemma SimpleGraph.Menger_inductive_step [Fintype V] (G : SimpleGraph V) [DecidableRel G.Adj] (A B : Set V) (x y : V) (hxy : G.Adj x y)
  ...
  : G.min_separator_size A B ≤ G.max_disjoint_paths_size A B := by
    ...
    · -- By the induction hypothesis on the contracted graph, there exists a set P' of disjoint paths in G/e with |P'| ≥ k.
      obtain ⟨P', hP'_card, hP'_disj⟩ : ∃ P' : Finset ((G.contractEdge' x y).ABPath (contractEdge_liftSet x y A) (contractEdge_liftSet x y B)), P'.card ≥ G.min_separator_size A B ∧ DisjointPaths P' := by
      ...
      have h_exists_P : ∃ P : Finset (G.ABPath A B), P.card = P'.card ∧ DisjointPaths P := by
...
theorem SimpleGraph.Menger_strong_aux (n : ℕ) :
  ∀ (V : Type u) [Fintype V] [DecidableEq V] (G : SimpleGraph V) [DecidableRel G.Adj] (A B : Set V),
...
-/
theorem SimpleGraph.Menger_strong [Fintype V] (G : SimpleGraph V) [DecidableRel G.Adj] (A B : Set V) :
  G.min_separator_size A B ≤ G.max_disjoint_paths_size A B := by
...
-/
theorem SimpleGraph.Menger [Fintype V] (G : SimpleGraph V) [DecidableRel G.Adj] (A B : Set V) :
  G.min_separator_size A B = G.max_disjoint_paths_size A B := by
