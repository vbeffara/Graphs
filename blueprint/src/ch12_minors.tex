...
\begin{proposition}[WQO criterion in Lean]
  \label{prop:12.1-criterion} \lean{WQO_iff} \leanok
...
\begin{lemma}[Diestel Lemma~12.1.3 (Higman)]
  \label{lem:12.1.3} \lean{Higman} \leanok
...
\begin{lemma}[Diestel Lemma~12.4.1]
  \label{lem:12.4.1} \lean{treeWidth_minor} \leanok
  \uses{def:12.4-treewidth,def:minor}
  If \(H \preceq G\), then \(\mathrm{tw}(H)\le \mathrm{tw}(G)\).
\end{lemma}

\begin{proof}
  \uses{lem:width-restrict,lem:width-map}
  \leanok
\end{proof}

\begin{theorem}[Diestel Theorem~12.4.2 (Robertson--Seymour 1990)]
  \label{thm:12.4.2}
  \uses{def:12.4-treewidth}
...
\begin{theorem}[Diestel Theorem~12.7.1 (Robertson--Seymour 1986--2004)]
  \label{thm:12.7.1} \lean{GraphMinorTheorem} \leanok
  \uses{def:minor}
...
\begin{theorem}[Planar forbidden-minor pointer]
  \label{thm:wagner} \lean{Wagner} \leanok
  \uses{def:minor}
