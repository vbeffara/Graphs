...
lemma SublistForall2_imp_FinsetLE (h : List.SublistForall₂ (· ≤ ·) s.toList t.toList) : FinsetLE s t := by
  -- Construct the injective function from s to t using the indices in t.toList that correspond to the elements in s.toList.
  -- have h_inj : ∃ f : s ↪ t, ∀ x : s, x.val ≤ f x := by
    have h_sublist : ∃ is : Fin s.card → Fin t.card, StrictMono is ∧ ∀ i, s.toList[i] ≤ t.toList[is i] := by
      have := h;
      have h_sublist : ∀ {l1 l2 : List α}, List.SublistForall₂ (· ≤ ·) l1 l2 → ∃ is : Fin l1.length ↪ Fin l2.length, StrictMono is ∧ ∀ i, l1[i] ≤ l2[is i] := by
        ...
        · rename_i l2 ih h_sublist ih';
          ...
          refine' ⟨ ⟨ fun i => Fin.cases ⟨ 0, _ ⟩ ( fun i => ⟨ is i + 1, _ ⟩ ) i, _ ⟩, _, _ ⟩ <;> simp_all +decide [ Fin.forall_fin_succ, StrictMono ];
          all_goals simp_all +decide [ Fin.forall_fin_succ, Function.Injective, StrictMono ];
          exacts [ fun i j hij => is.injective <| Fin.ext hij, fun i => Nat.succ_pos _, his₂ ];
        · rcases ‹_› with ⟨ is, his, h ⟩;
          refine' ⟨ ⟨ fun i => Fin.succ ( is i ), fun i j hij => _ ⟩, _, _ ⟩ <;> simp_all +decide [ StrictMono ];
      obtain ⟨ is, his ⟩ := h_sublist this;
