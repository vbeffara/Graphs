...
noncomputable abbrev ABPath.support (P : G.ABPath A B) : Finset V := P.p.1.support.toFinset
...
noncomputable def contractEdge_preimage [Fintype V] (x y : V)
    (Y : Finset (Quotient (contractEdgeSetoid x y))) : Set V :=
  Finset.univ.filter (⟦·⟧ ∈ Y)

lemma mem_contractEdge_preimage [Fintype V] {x y : V}
    {Y : Finset (Quotient (contractEdgeSetoid x y))} {v : V} :
...
-/
lemma card_preimage_contractEdge [Fintype V] (h : x ≠ y) (Y) : (contractEdge_preimage x y Y).toFinset.card =
    if contractEdge_vertex x y ∈ Y then Y.card + 1 else Y.card := by
  -- Let's count the number of elements in the preimage of Y.
  have h_card_preimage : (Finset.univ.filter (⟦·⟧ ∈ Y)).card =
      ∑ z ∈ Y, (Finset.univ.filter (⟦·⟧ = z)).card := by
    simp only [Finset.card_filter];
    rw [Finset.sum_comm, Finset.sum_congr rfl]
    simp
  -- Let's count the number of elements in each fiber of the projection map.
  have h_fiber_card : ∀ z ∈ Y, (Finset.univ.filter (⟦·⟧ = z)).card =
...
theorem SimpleGraph.contractEdge_separator_contains_vertex [Fintype V] (G : SimpleGraph V) (A B : Finset V) (x y : V) (k : ℕ)
  ...
  SimpleGraph.contractEdge_vertex x y ∈ Y.1 := by
    ...
    refine ⟨⟨SimpleGraph.contractEdge_preimage x y Y.1 |>.toFinset, ?_⟩, ?_⟩
...
lemma SimpleGraph.disjoint_paths_prop (G : SimpleGraph V) (A X : Finset V)
  ...
  ∀ x ∈ X, ∃! p ∈ P.1, p.v = x ∧ p.p.1.support.toFinset ∩ X = {x} := by
    ...
    have h_inj : Set.InjOn (fun p : G.ABPath A X => p.v.1) P.1 := by
      ...
      have := P.2 hp hq; simp_all [ Finset.disjoint_left ] ;
      contrapose! this;
    ...
    have := P.2 hp.1 hqP; simp_all [ Finset.disjoint_left ] ;
...
lemma SimpleGraph.ABPath_prefix_avoids_X (G : SimpleGraph V) (A X : Finset V) (X_fin : Finset V)
  (p : G.ABPath A X)
  (hp_X : p.p.1.support.toFinset ∩ X_fin = {p.v.1})
  (z : V)
  (hz : z ∈ p.p.1.support)
  (hzX : z ∉ X_fin) :
  (p.p.1.takeUntil z hz).support.toFinset ∩ X_fin = ∅ := by
    simp_all [ Finset.ext_iff ];
    intro a ha haX;
    have ha_support : a ∈ p.p.1.support := by
      have h_support_subset : (p.p.1.takeUntil z hz).support ⊆ p.p.1.support := by
        exact Walk.support_takeUntil_subset p.p.1 hz;
...
lemma SimpleGraph.ABPath_suffix_avoids_X (G : SimpleGraph V) (B X : Finset V) (X_fin : Finset V)
  (q : G.ABPath X B)
  (hq_X : q.p.1.support.toFinset ∩ X_fin = {q.u.1})
  (z : V)
  (hz : z ∈ q.p.1.support)
  (hzX : z ∉ X_fin) :
  (q.p.1.dropUntil z hz).support.toFinset ∩ X_fin = ∅ := by
    rw [ Finset.eq_empty_iff_forall_notMem ];
    intro x hx
    have hx_suff : x ≠ q.u := by
      intro hx_eq_q_u;
      have := q.p.1.start_notMem_support_dropUntil q.p.2 hz; simp_all ;
    simp_all [ Finset.ext_iff ];
    have := hq_X x; simp_all
    exact this ( q.p.1.support_dropUntil_subset hz hx.1 )
...
lemma SimpleGraph.path_intersection_of_separator (X : G.Separator A B) (p : G.ABPath A X.1)
    (q : G.ABPath X.1 B) (hp_X : p.p.1.support.toFinset ∩ X.1 = {p.v.1})
    (hq_X : q.p.1.support.toFinset ∩ X.1 = {q.u.1}) :
...
lemma SimpleGraph.disjoint_paths_prop_start (G : SimpleGraph V) (X B : Finset V)
  ...
  ∀ x ∈ X, ∃! p ∈ P.1, p.u = x ∧ p.p.1.support.toFinset ∩ X = {x} := by
    ...
    have h_distinct_start : ∀ p q : G.ABPath X B, p ∈ P.1 → q ∈ P.1 → p ≠ q → p.u.1 ≠ q.u.1 := by
      intro p q hp hq hpq h; have := P.2 hp hq; simp_all
      exact this ( p.p.1.start_mem_support ) ( by simp [ h ] );
    ...
    have h_path_intersects_X_only_at_start : ∀ x ∈ X, ∀ p ∈ P.1, p.u = x → p.p.1.support.toFinset ∩ X = {x} := by
      intro x hx p hp hp_start
      have h_path_intersects_X_only_at_start : ∀ z ∈ p.p.1.support, z ∈ X → z = x := by
        intro z hz hzX
        by_contra h_contra;
...
lemma SimpleGraph.joined_path_is_path (G : SimpleGraph V) (A B : Finset V)
  (X : G.Separator A B)
  (x : V)
  (p : G.ABPath A X.1) (h_p : p.v = x) (h_p_X : p.p.1.support.toFinset ∩ X.1 = {x})
  (q : G.ABPath X.1 B) (h_q : q.u = x) (h_q_X : q.p.1.support.toFinset ∩ X.1 = {x}) :
  ((p.p.1.copy rfl h_p).append (q.p.1.copy h_q rfl)).IsPath := by
    ...
    have h_support_union : (p.p.1.copy rfl h_p).support.toFinset ∩ (q.p.1.copy h_q rfl).support.toFinset ⊆ {x} := by
...
lemma SimpleGraph.joined_paths_disjoint (G : SimpleGraph V) (A B : Finset V) (X : G.Separator A B)
  (x y : V) (hxy : x ≠ y)
  (px : G.ABPath A X.1) (hpx : px.v = x) (hpx_X : px.p.1.support.toFinset ∩ X.1 = {x})
  (py : G.ABPath A X.1) (hpy : py.v = y) (hpy_X : py.p.1.support.toFinset ∩ X.1 = {y})
  (qx : G.ABPath X.1 B) (hqx : qx.u = x) (hqx_X : qx.p.1.support.toFinset ∩ X.1 = {x})
  (qy : G.ABPath X.1 B) (hqy : qy.u = y) (hqy_X : qy.p.1.support.toFinset ∩ X.1 = {y})
  (hp_disj : Disjoint px.p.1.support.toFinset py.p.1.support.toFinset)
  (hq_disj : Disjoint qx.p.1.support.toFinset qy.p.1.support.toFinset) :
  Disjoint (px.p.1.support.toFinset ∪ qx.p.1.support.toFinset) (py.p.1.support.toFinset ∪ qy.p.1.support.toFinset) := by
    norm_num +zetaDelta at *;
    have hpqx : ∀ w ∈ px.p.1.support, w ∉ qy.p.1.support := by
      have := path_intersection_of_separator X px qy; simp_all [ Finset.ext_iff ] ;
    have hqypx : ∀ w ∈ qx.p.1.support, w ∉ py.p.1.support := by
      intro w hw hw';
...
theorem SimpleGraph.disjoint_paths_join (G : SimpleGraph V) (A B : Finset V) (X : G.Separator A B) (k : ℕ)
  ...
  (hP_B_card : P_B.1.card = k) : ∃ P : G.Joiner A B, P.1.card = k := by
    have h_unique_paths : ∀ x ∈ X.1, ∃! p ∈ P_A.1, p.v = x ∧ p.p.1.support.toFinset ∩ X.1 = {x} := by
      have := SimpleGraph.disjoint_paths_prop G A ( ↑X.1 ) P_A; aesop;
    have h_unique_paths_start : ∀ x ∈ X.1, ∃! q ∈ P_B.1, q.u = x ∧ q.p.1.support.toFinset ∩ X.1 = {x} := by
    ...
    · apply p x x.2 |>.p.1.copy rfl (hp x x.2 |>.2.1) |>.append
      exact q x x.2 |>.p.1.copy ( hq x x.2 |>.2.1 ) rfl
    · refine
      ...
      · simp [hq]
    · intro p hp q hq hpq;
      rw [ SetLike.mem_coe, Finset.mem_image ] at hp hq
      obtain ⟨x, hx, rfl⟩ := hp
      obtain ⟨y, hy, rfl⟩ := hq
      generalize_proofs at *;
      by_cases hxy : x = y;
      ...
      · convert SimpleGraph.joined_paths_disjoint G A B X x y ( by simpa ) ( p x x.2 ) ( hp x x.2 |>.2.1 ) ( hp x x.2 |>.2.2 ) ( p y y.2 ) ( hp y y.2 |>.2.1 ) ( hp y y.2 |>.2.2 ) ( q x x.2 ) ( hq x x.2 |>.2.1 ) ( hq x x.2 |>.2.2 ) ( q y y.2 ) ( hq y y.2 |>.2.1 ) ( hq y y.2 |>.2.2 ) _ _ using 1;
        · simp [ Finset.ext_iff ];
        · simp [ Finset.ext_iff ];
        · have := P_A.2 ( hp x x.2 |>.1 ) ( hp y y.2 |>.1 ) ; simp_all [ Finset.disjoint_left ] ;
          grind;
        · have := P_B.2 ( hq x x.2 |>.1 ) ( hq y y.2 |>.1 ) ; simp_all [ Finset.disjoint_left ] ;
          exact this ( by intro h; have := hq x x.2; have := hq y y.2; aesop );
    · simp [ hX_card ];
    · intro x hx y hy hxy;
      simp_all [ Finset.ext_iff ]
      have := P_A.2 ( hp x x.2 |>.1 ) ( hp y y.2 |>.1 )
...
lemma SimpleGraph.contractEdge_separator_lift_card [Fintype V] (x y : V) (hxy : x ≠ y)
  ...
  (SimpleGraph.contractEdge_preimage x y Y).toFinset.card = Y.card + 1 := by
...
lemma SimpleGraph.support_subset_preimage_extension [Fintype V] (x y : V)
  ...
  (h_subset : q_support.image (⟦·⟧) ⊆ p'_support)
  (h_ve : SimpleGraph.contractEdge_vertex x y ∈ p'_support)
  (hv : v = x ∨ v = y) :
  (q_support ∪ {v}) ⊆ (SimpleGraph.contractEdge_preimage x y p'_support).toFinset := by
...
lemma SimpleGraph.lift_path_to_contraction_end [Fintype V] (G : SimpleGraph V) (A : Finset V) (x y : V)
  ...
  ∃ (u v : V) (p : G.Walk u v),
    ...
    (v = x ∨ v = y) ∧
    p.IsPath ∧
    p.support.toFinset ⊆ (SimpleGraph.contractEdge_preimage x y p'.support.toFinset).toFinset ∧
    p.support.toFinset ∩ {x, y} = {v} := by
      norm_num +zetaDelta at *;
      ...
      obtain ⟨u, w, q, hu, hw, hq_path, hq_support⟩ : ∃ u w : V, ∃ q : G.Walk u w,
          u ∈ A ∧ ⟦u⟧ = u' ∧ ⟦w⟧ = w' ∧ q.IsPath ∧ q.support.toFinset.image (⟦·⟧) ⊆ q'.support.toFinset ∧
          x ∉ q.support ∧ y ∉ q.support := by
      ...
      have h_support_subset_preimage : p.support.toFinset ⊆
          (SimpleGraph.contractEdge_preimage x y p'.support.toFinset).toFinset := by
  ...
  ∃ (u v : V) (p : G.Walk u v),
    ...
    p.support.toFinset ∩ {x, y} = {u} := by
      have h_lift_reversed : ∃ u v : V, ∃ p : G.Walk u v, u ∈ B ∧
        (v = x ∨ v = y) ∧
        p.IsPath ∧
        p.support.toFinset ⊆ (contractEdge_preimage x y (p'.reverse.support.toFinset)).toFinset ∧
        p.support.toFinset ∩ {x, y} = {v} := by
...
lemma SimpleGraph.contractEdge_preimage_disjoint_away_from_endpoints [Fintype V] (x y : V)
  ...
  Disjoint ((SimpleGraph.contractEdge_preimage x y s |>.toFinset) \ {x, y}) ((SimpleGraph.contractEdge_preimage x y t |>.toFinset) \ {x, y}) := by
    refine Finset.disjoint_left.mpr ?_
    intro a ha hb
    rcases Finset.mem_sdiff.mp ha with ⟨ha_pre, ha_not⟩
    rcases Finset.mem_sdiff.mp hb with ⟨hb_pre, _⟩
    simp at ha_pre hb_pre
    have hproj_ne : ⟦a⟧ ≠ SimpleGraph.contractEdge_vertex x y := by
      ...
      exact ha_not (by simpa [Finset.mem_insert, Finset.mem_singleton] using hxy)
    have ha_s : ⟦a⟧ ∈ s :=
      (mem_contractEdge_preimage (x := x) (y := y) (Y := s) (v := a)).1 ha_pre
    have hb_t : ⟦a⟧ ∈ t :=
      (mem_contractEdge_preimage (x := x) (y := y) (Y := t) (v := a)).1 hb_pre
    have ha_s' : ⟦a⟧ ∈ s \ {SimpleGraph.contractEdge_vertex x y} := by
      refine Finset.mem_sdiff.mpr ?_
      exact ⟨ha_s, by simpa [Finset.mem_singleton] using hproj_ne⟩
    have hb_t' : ⟦a⟧ ∈ t \ {SimpleGraph.contractEdge_vertex x y} := by
      refine Finset.mem_sdiff.mpr ?_
      exact ⟨hb_t, by simpa [Finset.mem_singleton] using hproj_ne⟩
    exact (Finset.disjoint_left.mp h_disj) ha_s' hb_t'
...
lemma SimpleGraph.lifted_paths_disjoint [Fintype V] (G : SimpleGraph V) (x y : V)
  ...
  (h_sub1 : p1.support.toFinset ⊆ (SimpleGraph.contractEdge_preimage x y p1'.support.toFinset).toFinset)
  (h_sub2 : p2.support.toFinset ⊆ (SimpleGraph.contractEdge_preimage x y p2'.support.toFinset).toFinset) :
  Disjoint (p1.support.toFinset \ {x, y}) (p2.support.toFinset \ {x, y}) := by
    ...
    have h_disjoint : Disjoint ((SimpleGraph.contractEdge_preimage x y p1'.support.toFinset).toFinset \ {x, y}) ((SimpleGraph.contractEdge_preimage x y p2'.support.toFinset).toFinset \ {x, y}) := by
...
lemma SimpleGraph.lift_split_paths [Fintype V] (G : SimpleGraph V) (A B : Finset V) (x y : V)
  ...
  ∃ (u_start u_end : V) (p1 : G.Walk u_start u_end) (v_start v_end : V) (p2 : G.Walk v_start v_end),
    ...
    p1.IsPath ∧ p2.IsPath ∧
    p1.support.toFinset ⊆ (SimpleGraph.contractEdge_preimage x y p1'.support.toFinset).toFinset ∧
    p2.support.toFinset ⊆ (SimpleGraph.contractEdge_preimage x y p2'.support.toFinset).toFinset ∧
    ...
    Disjoint (p1.support.toFinset \ {x, y}) (p2.support.toFinset \ {x, y}) := by
      ...
      have h_lifted_paths_disjoint : Disjoint ((SimpleGraph.contractEdge_preimage x y p1'.support.toFinset).toFinset \ {x, y}) ((SimpleGraph.contractEdge_preimage x y p2'.support.toFinset).toFinset \ {x, y}) := by
...
lemma SimpleGraph.lift_path_through_contraction_internal [Fintype V] (G : SimpleGraph V) (A B : Finset V) (x y : V) (hxy : G.Adj x y)
  ...
  ∃ (u v : V) (p : G.Walk u v),
    u ∈ A ∧ v ∈ B ∧
    p.IsPath ∧
    p.support.toFinset ⊆ (SimpleGraph.contractEdge_preimage x y p'.support.toFinset).toFinset := by
      ...
      have h_union_subset : (contractEdge_preimage x y p1'.support.toFinset).toFinset ∪
        (contractEdge_preimage x y p2'.support.toFinset).toFinset ⊆ (contractEdge_preimage x y (p1'.support.toFinset ∪ p2'.support.toFinset)).toFinset := by
        simp [ Finset.subset_iff, SimpleGraph.contractEdge_preimage ];
      refine ⟨u_start, v_end, q, hp1, hp2, hq.1, hq.2.trans ?_⟩
      simp_all
      grind
...
lemma SimpleGraph.lift_path_start_eq_vertex [Fintype V] (G : SimpleGraph V) (A B : Finset V) (x y : V) (hxy : G.Adj x y)
  ...
  ∃ p : G.ABPath A B,
    p.p.1.support.toFinset.image (⟦·⟧) ⊆ p'.support.toFinset := by
      ...
      have hw' : w ∈ (SimpleGraph.contractEdge_preimage x y p'.support.toFinset).toFinset := hq_pre hw
      refine (SimpleGraph.mem_contractEdge_preimage (x := x) (y := y) (Y := p'.support.toFinset) (v := w)).1 ?_
      simpa using hw'
...
lemma SimpleGraph.lift_path_end_eq_vertex [Fintype V] (G : SimpleGraph V) (A B : Finset V) (x y : V) (hxy : G.Adj x y)
  ...
  ∃ p : G.ABPath A B,
    p.p.1.support.toFinset.image (⟦·⟧) ⊆ p'.support.toFinset := by
      ...
      obtain ⟨ v', q, hv', hq, hq' ⟩ : ∃ v' : V, ∃ q : G.Walk u v', v' ∈ B ∧ q.IsPath ∧
        (q.support.toFinset.image (⟦·⟧) : Finset (Quotient (contractEdgeSetoid x y)))
          ⊆ p.support.toFinset.image (⟦·⟧) := by
      ...
      have h_final : Finset.image (⟦·⟧) q.support.toFinset ⊆ p'.support.toFinset := by
        ...
        intro z hz
        have hz' : z ∈ SimpleGraph.contractEdge_preimage x y p'.support.toFinset := by simpa using hp' hz
    ...
    p.p.1.support.toFinset.image (⟦·⟧) ⊆ p'.p.1.support.toFinset := by
      ...
      · cases' p' with u' hv' p;
        ...
        · convert SimpleGraph.lift_path_end_eq_vertex G A B x y hxy _ _ _ _ _ _;
          ...
          · grind;
        · rename_i p hp;
          obtain ⟨ p, hp ⟩ := SimpleGraph.lift_path_through_contraction_internal G A B x y hxy u' v' p p.2 hp'_mem hu' hv' u'.2 ‹_›;
...
lemma SimpleGraph.exists_disjoint_paths_lift [Fintype V] (G : SimpleGraph V) (A B : Finset V) (x y : V)
  ...
  ∃ P : G.Joiner A B, P.1.card = P'.1.card := by
    ...
    · intro p hp p' hp' hpp';
      ...
      intro v hv hv'; have := this hq hq'; simp_all [ Finset.disjoint_left ] ;
    ...
    · rw [ Finset.card_image_of_injOn ];
      ...
      contrapose! h_eq;
      have h_support_disjoint : Disjoint (p'.p.1.support.toFinset) (q'.p.1.support.toFinset) := by
        have := P'.2 hp' hq' h_eq; simp_all [ Finset.disjoint_left ] ;
      have h_support_disjoint_lift : Disjoint (f p').p.1.support.toFinset (f q').p.1.support.toFinset := by
        refine' Finset.disjoint_left.mpr _;
        intro a ha hb; have := hf p' a; have := hf q' a; simp_all [ Finset.disjoint_left ] ;
        grind;
      intro h; simp_all [ Finset.disjoint_left ] ;
      exact h_support_disjoint_lift ( h.symm ▸ ( f q' ).p.1.start_mem_support ) ( ( f q' ).p.1.start_mem_support )
...
lemma SimpleGraph.Menger_case2_exists_X [Fintype V] (G : SimpleGraph V) (A B : Finset V) (x y : V) (hxy : x ≠ y)
  ...
  ∃ X : G.Separator A B, X.1.card = k ∧ x ∈ X.1 ∧ y ∈ X.1 := by
    ...
    obtain ⟨⟨X, hX_sep⟩, hX_card⟩ : ∃ X : G.Separator A B, X.1.card = Y.1.card + 1 ∧ x ∈ X.1 ∧ y ∈ X.1 := by
      ...
      refine' ⟨⟨_, this⟩, _, _, _ ⟩ <;> simp_all
      · have := SimpleGraph.contractEdge_separator_lift_card x y hxy Y this
        simpa [contractEdge_preimage] using this
...
lemma SimpleGraph.lift_disjoint_paths_le (G G' : SimpleGraph V) (h : G' ≤ G) (A B : Finset V)
  ...
  · intro p hp q hq hpq
    ...
    have hpq' : p' ≠ q' := by
      ...
      apply hpq
      simp [f, h']
    have hdisj0 : Disjoint (p'.p.1.support.toFinset) (q'.p.1.support.toFinset) := hP hp' hq' hpq'
    have hdisj : Disjoint ((p'.p.1.mapLe h).support.toFinset) ((q'.p.1.mapLe h).support.toFinset) := by
      simpa [SimpleGraph.Walk.support_mapLe_eq_support] using hdisj0
    exact hdisj
  · apply Finset.card_image_of_injOn
    intro p hp q hq hpq
    by_contra hneq
    have hdisj : Disjoint p.p.1.support.toFinset q.p.1.support.toFinset := hP hp hq hneq
    ...
    have hp_mem : (p.u : V) ∈ p.p.1.support.toFinset := by
      simp [p.p.1.start_mem_support]
    have hq_mem : (p.u : V) ∈ q.p.1.support.toFinset := by
...
lemma SimpleGraph.Menger_case2_imp_paths (G : SimpleGraph V) (A B : Finset V) (x y : V) (hxy : G.Adj x y)
  ...
  obtain ⟨⟨P_A, hP_A_disj⟩, hP_A_card⟩ : ∃ P_A : G.Joiner A X.1, P_A.1.card = k := by
    ...
    have h_lift_A : ∃ P_A : G.Joiner A X.1, P_A.1.card = t.card := by
      ...
      · exact fun p hp q hq hpq => hP_A'_disj ( ht₁ hp ) ( ht₁ hq ) hpq;
    simp_rw [← ht₂]
    exact h_lift_A
  obtain ⟨⟨P_B'', hP_B''_disj⟩, hP_B''_card⟩ : ∃ P_B'' : (G.deleteEdge x y).Joiner X.1 B, P_B''.1.card = k := by
    obtain ⟨ P_B'', hP_B''_disj, hP_B''_card ⟩ := Finset.exists_subset_card_eq hP_B'_card;
    exact ⟨⟨P_B'', fun p hp q hq hpq => hP_B'_disj ( hP_B''_disj hp ) ( hP_B''_disj hq ) hpq⟩, hP_B''_card ⟩;
